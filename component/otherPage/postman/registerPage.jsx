...
const RegisterPage = () => {
  ...
  const handleSubmitForm = (event) => {
    ...
    setWarning(false);
    otherPageService.gmailRegister(account, password).then((response) => {
      ...
      <div style={{ marginTop: "7em", textAlign: "center" }}>
        ...
        >
          <ol>
            <li>
              {" "}
              Get your application password first, please see the driscription
              on google to get that password, and{" "}
              <span style={{ color: "red" }}>REMEMBER</span> it. Here's the link
              to google page.{" "}
              <a
              ...
              >
                {" "}
                <button className="btn btn-dark btn-sm">Link</button>{" "}
              </a>{" "}
            </li>
            <li style={{ marginTop: "1em" }}>
              {" "}
              Fill in the input area with your Google account (Both with or
              without @gmail.com are fine) and the application password you just
              got. Press Register, if successful, the button color will turn
              green, done!
            </li>
            <li style={{ marginTop: "1em" }}>
              {" "}
              After the registration, you won't be asked to input the
              application password any longer.{" "}
            </li>
          </ol>
          <div></div>
          <span style={{ textAlign: "right" }}>
            <form style={{ marginTop: "3em" }} onSubmit={handleSubmitForm}>
              ...
              <label style={{ padding: "0.5em" }}>
                <button
                  className={registered ? "btn btn-success" : "btn btn-info"}
                  style={{
                    width: "120px",
                    height: "120px",
                    borderRadius: "50%",
        ...
        {registered ? <p>Redirecting... {counting} seconds</p> : <></>}

        {warning ? (
          <div className="alert alert-danger" style={{ marginTop: "2em" }}>
            {" "}
            Gmail Account and Password can't be empty!{" "}
          </div>
        ) : (
          <></>
        )}
